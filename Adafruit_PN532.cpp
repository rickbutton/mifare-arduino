...
boolean Adafruit_PN532::SetCRC(boolean enable) {
  #ifdef PN532DEBUG
    Serial.println("SetCRC");
  #endif

  if (!WriteRegister(0x63, 0x03, enable ? 0x80 : 0x00))
    return false;

  return WriteRegister(0x63, 0x02, enable ? 0x80 : 0x00);
}

boolean Adafruit_PN532::SetParity(boolean enable) {
  #ifdef PN532DEBUG
    Serial.println("SetParity");
  #endif

  return WriteRegister(0x63, 0x0d, enable ? 0x00 : 0x10);
}

boolean Adafruit_PN532::SetParameters(uint8_t value) {
  #ifdef PN532DEBUG
    Serial.println("SetParameters");
  #endif

  pn532_packetbuffer[0] = PN532_COMMAND_SETPARAMETERS;
  pn532_packetbuffer[1] = value;

  if (! sendCommandCheckAck(pn532_packetbuffer, 2, 0))
    return false;

  readspidata(pn532_packetbuffer, 8);
}

boolean Adafruit_PN532::ReadRegister(uint8_t high, uint8_t low, uint8_t *value) {
  #ifdef PN532DEBUG
    Serial.println("WriteRegister");
  #endif

  pn532_packetbuffer[0] = PN532_COMMAND_READREGISTER;
  pn532_packetbuffer[1] = high;
  pn532_packetbuffer[2] = low;

  if (! sendCommandCheckAck(pn532_packetbuffer, 3, 0))
    return false;

  readspidata(pn532_packetbuffer, 8);
  *value = pn532_packetbuffer[6];
}

boolean Adafruit_PN532::WriteRegister(uint8_t high, uint8_t low, uint8_t value) {
  #ifdef PN532DEBUG
    Serial.println("WriteRegister");
  #endif
  ...
  if (! sendCommandCheckAck(pn532_packetbuffer, 4, 0))
    return false;

  readspidata(pn532_packetbuffer, 8);
}

boolean Adafruit_PN532::GetGeneralStatus() {
  #ifdef PN532DEBUG
    Serial.println("GetGeneralStatus");
  #endif
  pn532_packetbuffer[0] = PN532_COMMAND_GETGENERALSTATUS;

  if (! sendCommandCheckAck(pn532_packetbuffer, 1, 0))
    return false;
...
boolean Adafruit_PN532::GetFIFOBuffer() {
  #ifdef PN532DEBUG
    Serial.println("GetGeneralStatus");
  #endif
  ...
  for (uint8_t i = 0; i < 12; i++) {
    uint8_t val = 0xFF;
    boolean b = ReadRegister(0x63, 0x39, &val);
    Serial.println(val, HEX);
  }
}

boolean Adafruit_PN532::TgGetTargetStatus() {
  #ifdef PN532DEBUG
    Serial.println("TgGetTargetStatus");
  #endif
  pn532_packetbuffer[0] = PN532_COMMAND_TGGETTARGETSTATUS;

  if (! sendCommandCheckAck(pn532_packetbuffer, 1, 0))
    return false;

  readspidata(pn532_packetbuffer, 9);

  return true;
}

boolean Adafruit_PN532::TgInitAsTarget(byte *rx, uint8_t rxSize) {
  pn532_packetbuffer[0] = PN532_COMMAND_TGINITASTARGET;
  
  pn532_packetbuffer[1] = 0x00; // 14443-3

  pn532_packetbuffer[2] = 0x0f; // SENS_RES
  pn532_packetbuffer[3] = 0x01;

  pn532_packetbuffer[4] = 0x12; // NFCID1
  pn532_packetbuffer[5] = 0x34; // NFCID1
  pn532_packetbuffer[6] = 0x56; // NFCID1

  pn532_packetbuffer[7] = 0x08; // SEL_RES

  memset(pn532_packetbuffer + 8, 0, 30); // felicia + nfcid3t + gen + hist


  if (! sendCommandCheckAck(pn532_packetbuffer, 38, 0))
    return false;

  readspidata(pn532_packetbuffer, 12);

  //memcpy(rx, pn532_packetbuffer + 8, rxSize);

  return true;
}

boolean Adafruit_PN532::TgResponseToInitiator(byte *tx, uint8_t txSize) {
  #ifdef PN532DEBUG
    Serial.println("TgResponseToInitiator");
  #endif
  pn532_packetbuffer[0] = PN532_COMMAND_TGRESPONSETOINITIATOR;

  memcpy(pn532_packetbuffer + 1, tx, txSize);
  
  if (! sendCommandCheckAck(pn532_packetbuffer, 1 + txSize, 0))
    return false;
...
boolean Adafruit_PN532::TgGetInitiatorCommand(byte *rx, uint8_t rxSize) {
  #ifdef PN532DEBUG
  ...
  readspidata(pn532_packetbuffer, 16);
  PrintHexChar(pn532_packetbuffer, 16);

  memcpy(rx, pn532_packetbuffer + 7, rxSize);
  return true;
}
